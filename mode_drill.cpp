...
void ModeDrill::onButton(uint8_t btn) {
  ...
  if (_phase == DrillPhase::RESULT) {
    switch (btn) {
      ...
      case 1:
        _phase = DrillPhase::REVIEW;
        UIScreen::resetShotPage();  // 第一次進入 REVIEW 從頭開始
        UIScreen::setTotalShotUI(min((uint32_t)_result.record.hit_count,
                                     (uint32_t)_def.shotCount));
        drawReviewScreen();
        break;  // SHOTS
  ...
  if (_phase == DrillPhase::REVIEW) {
    switch (btn) {
      case 0:  // BtnA — 上一頁 or 返回結果頁
        if (UIScreen::shotPageHasPrev()) {
          UIScreen::shotPagePrev();
          drawReviewScreen();
        } else {
          _phase = DrillPhase::RESULT;
          drawResultScreen();
        }
        break;
      case 1:  // BtnB — 返回結果頁
        ...
        drawResultScreen();
        break;
      case 2:  // BtnC — 下一頁 or 返回結果頁
        if (UIScreen::shotPageHasNext()) {
...
void ModeDrill::drawReviewScreen() {
  ...
  for (uint8_t i = 0; i < rec.hit_count && i < _def.shotCount; i++) {
    uint32_t hitIdx = i + 1;
    if (hitIdx < pageBase || hitIdx >= pageEnd) continue;
    unsigned long elapsed = rec.hits[i].hit_time_ms - rec.start_time_ms;
    unsigned long split   = (i == 0) ? elapsed
                           : rec.hits[i].hit_time_ms - rec.hits[i-1].hit_time_ms;
    bool pass = judgeShot(hitIdx, split);
    UIScreen::drawShotRowAt(hitIdx, pageBase, rec.hits[i].station_id, elapsed, split, pass);
  }

  // BtnA: 有上頁→PREV，無上頁→RESULT
  // BtnB: 永遠 RESULT
  // BtnC: 有下頁→NEXT，無下頁→RESULT
  const char* lblA = UIScreen::shotPageHasPrev() ? "< PREV" : "RESULT";
  const char* lblC = UIScreen::shotPageHasNext() ? "NEXT >" : "RESULT";
  UIScreen::drawBtnBar(lblA, "RESULT", lblC);
}
 ...
 */
bool ModeDrill::judgeShot(uint32_t hitIdx, unsigned long split) const {
 ...
 * @param split     分段時間（ms）
 */
void ModeDrill::onHit(uint32_t hitIdx, uint8_t stationId,
