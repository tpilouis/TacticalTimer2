...
void UIScreen::drawModeHeader(const char* title, const char* badge, uint16_t badgeColor) {
  // 清除標題列
  M5.Lcd.fillRect(0, Layout::GAME_AREA_Y, HW::LCD_W, Layout::MODE_HDR_H, UIColor::CARD_BG);

  M5.Lcd.setFreeFont(FSS9);
  M5.Lcd.setTextDatum(ML_DATUM);
  M5.Lcd.setTextColor(TFT_CYAN, UIColor::CARD_BG);
  ...
  if (badge) {
    ...
    // 先清除 badge 區域（fillRect 覆蓋圓角殘影）
    M5.Lcd.fillRect(bx - 1, by - 1, bw + 2, bh + 2, UIColor::CARD_BG);
    drawBadge(bx, by, bw, bh, badge, badgeColor, TFT_BLACK);
  }
}
 ...
 */
void UIScreen::drawShotRow(uint8_t hitIdx, uint8_t stationId,
                           unsigned long elapsed, unsigned long split,
                           bool pass) {
  TT2_BOUNDS(hitIdx, Limits::HIT_MAX + 1, return);

  // ── 佈局常數 ────────────────────────────────────────────────
  constexpr int16_t START_Y   = Layout::GAME_AREA_Y + Layout::MODE_HDR_H + 2;
  constexpr int16_t STATS_H   = 50;
  constexpr int16_t DIVIDER   = 2;
  constexpr int16_t SHOT_END_Y= Layout::IP_Y - STATS_H - DIVIDER;  // 152
  constexpr int16_t SHOT_AREA = SHOT_END_Y - START_Y;               // 112
  constexpr int16_t ROW_H     = SHOT_AREA / 5;                      // 22
  constexpr int16_t GAP       = 8;                                    // 中間分隔 gap
  constexpr int16_t COL_W     = HW::LCD_W / 2 - GAP / 2 - 2;        // 154px

  // 左欄 1–5，右欄 6–10
  bool    rightCol = (hitIdx > 5);
  uint8_t localIdx = rightCol ? (hitIdx - 6) : (hitIdx - 1);

  // 左欄 x0=2，右欄 x0=166（多推 4px，中間共 8px gap）
  int16_t x0 = rightCol ? (HW::LCD_W / 2 + GAP / 2 + 2) : 2;
  int16_t y0 = START_Y + localIdx * ROW_H;
  int16_t yC = y0 + ROW_H / 2;   // 列垂直中心

  // 底色（含清除中間 gap 殘影）
  M5.Lcd.fillRect(x0, y0, COL_W, ROW_H - 1, TFT_BLACK);
  if (!rightCol) {
    // 左欄：清除右側 gap 區域
    M5.Lcd.fillRect(x0 + COL_W, y0, GAP, ROW_H - 1, TFT_BLACK);
  }

  // ── FSS9（FreeSans 9pt）字高 ~11px，字寬 ~7px ────────────────
  // COL_W=158px 分配：
  //   序號   右對齊 +16   (2字元×7px+2gap=16)
  //   badge  +20 ~ +50   (30px，badge中心@35)
  //   elapsed 右對齊 +104  (空間 54px)
  //   split   右對齊 +154  (空間 50px，右邊留 4px)
  M5.Lcd.setFreeFont(FSS9);
  M5.Lcd.setTextSize(1);
  ...
  M5.Lcd.setTextColor(UIColor::TEXT_DIM, TFT_BLACK);
  char numBuf[4]; snprintf(numBuf, sizeof(numBuf), "%02u", hitIdx);
  M5.Lcd.drawString(numBuf, x0 + 16, yC, GFXFF);

  // 來源徽章（30px 寬，badge 左邊 +20，右邊 +50）
  ...
  M5.Lcd.fillRoundRect(x0 + 20, y0 + 3, 30, ROW_H - 6, 2, srcBg);
  M5.Lcd.setTextDatum(MC_DATUM);
  M5.Lcd.setTextColor(srcCol, srcBg);
  M5.Lcd.drawString(srcBuf, x0 + 35, yC, GFXFF);  // badge 中心 20+15=35

  // 累計時間（右對齊到 x0+104）
  char elBuf[10]; fmtTime(elapsed, elBuf, sizeof(elBuf));
  M5.Lcd.setTextDatum(MR_DATUM);
  M5.Lcd.setTextColor(TFT_WHITE, TFT_BLACK);
  M5.Lcd.drawString(elBuf, x0 + 104, yC, GFXFF);
  ...
  M5.Lcd.setTextDatum(MR_DATUM);
  M5.Lcd.drawString(spBuf, x0 + COL_W - 4, yC, GFXFF);
}
...
void UIScreen::drawSpyScreen(uint8_t shotCount, unsigned long totalMs,
  ...
  constexpr int16_t GAP   = 8;                        // 中間分隔
  constexpr int16_t CW    = HW::LCD_W / 2 - GAP / 2; // 欄寬 ~156px
  constexpr int16_t RX    = HW::LCD_W / 2 + GAP / 2; // 右欄起始 X
  ...
  // ── 頂部摘要列 ───────────────────────────────────────────────
  M5.Lcd.fillRect(0, Y0, HW::LCD_W, ROW_H, TFT_BLACK);
  M5.Lcd.setTextDatum(TL_DATUM);
  char buf[24];
  snprintf(buf, sizeof(buf), "Shots: %u / %u", shotCount, Limits::HIT_MAX);
  M5.Lcd.setTextColor(UIColor::TEXT_DIM, TFT_BLACK);
  M5.Lcd.drawString(buf, 4, Y0, GFXFF);

  if (shotCount > 0 && totalMs > 0) {
    char totBuf[10]; fmtTime(totalMs, totBuf, sizeof(totBuf));
    snprintf(buf, sizeof(buf), "Tot:%ss", totBuf);
    M5.Lcd.setTextDatum(TR_DATUM);
    M5.Lcd.setTextColor(TFT_WHITE, TFT_BLACK);
    M5.Lcd.drawString(buf, HW::LCD_W - 4, Y0, GFXFF);
  }

  // ── 分隔線 ────────────────────────────────────────────────────
  constexpr int16_t LIST_Y = Y0 + ROW_H + 2;
  M5.Lcd.drawFastHLine(0, LIST_Y - 1, HW::LCD_W, UIColor::CARD_BORD);
  // 中間垂直分隔
  M5.Lcd.drawFastVLine(HW::LCD_W / 2, LIST_Y, ROWS * ROW_H, UIColor::CARD_BORD);

  // ── 左欄 #1~5，右欄 #6~10 ────────────────────────────────────
  for (uint8_t col = 0; col < 2; col++) {
    int16_t x0 = (col == 0) ? 2 : RX + 2;
    for (uint8_t row = 0; row < ROWS; row++) {
      uint8_t idx = col * ROWS + row;  // 0-based
      int16_t y   = LIST_Y + row * ROW_H;

      // 清除該列
      M5.Lcd.fillRect(x0, y, CW - 2, ROW_H - 1, TFT_BLACK);

      // 序號（暗色）
      M5.Lcd.setTextDatum(ML_DATUM);
      M5.Lcd.setTextColor(UIColor::TEXT_DIM, TFT_BLACK);
      char idxBuf[4]; snprintf(idxBuf, sizeof(idxBuf), "#%u", idx + 1);
      M5.Lcd.drawString(idxBuf, x0, y + ROW_H / 2, GFXFF);

      if (idx < shotCount) {
        char spBuf[10]; fmtTime(hits[idx].split, spBuf, sizeof(spBuf));
        if (idx == 0) {
          snprintf(buf, sizeof(buf), "%ss", spBuf);   // 第一發：0.00s（無+號）
        } else {
          snprintf(buf, sizeof(buf), "+%ss", spBuf);  // 其後：+X.XXs
        }
        M5.Lcd.setTextDatum(MR_DATUM);
        M5.Lcd.setTextColor(idx == 0 ? TFT_CYAN : TFT_WHITE, TFT_BLACK);
        M5.Lcd.drawString(buf, x0 + CW - 4, y + ROW_H / 2, GFXFF);
      } else {
        // 無資料：顯示虛線
        M5.Lcd.setTextDatum(MR_DATUM);
        M5.Lcd.setTextColor(UIColor::CARD_BORD, TFT_BLACK);
        M5.Lcd.drawString("---", x0 + CW - 4, y + ROW_H / 2, GFXFF);
      }
    }
  }
}
...
void UIScreen::drawHistoryDetail(const char* path, const GameRecord& rec,
  ...
  constexpr int16_t CW    = HW::LCD_W / 2 - GAP / 2;
  constexpr int16_t RX    = HW::LCD_W / 2 + GAP / 2;
  ...
  if (drillDef && drillDef->name[0]) {
    // DRILL mode：顯示課題名稱 + par time
    char hdr[32];
    char parBuf[10]; fmtTime(drillDef->parTimeMs, parBuf, sizeof(parBuf));
    snprintf(hdr, sizeof(hdr), "%s  par:%ss", drillDef->name, parBuf);
    M5.Lcd.drawString(hdr, 4, Y0, GFXFF);
  } else {
    M5.Lcd.drawString(fn, 4, Y0, GFXFF);
  }

  uint8_t hits = rec.hit_count < Limits::HIT_MAX ? rec.hit_count : Limits::HIT_MAX;
  bool twoCol  = (hits > 6);
  uint8_t col1 = twoCol ? 5 : hits;
  uint8_t col2 = twoCol ? hits - 5 : 0;

  constexpr int16_t LIST_Y = Y0 + ROW_H + 2;

  // Pass/Fail 判斷 threshold（DRILL mode）
  ...
  if (twoCol)
    M5.Lcd.drawFastVLine(HW::LCD_W / 2, LIST_Y, col1 * ROW_H, UIColor::CARD_BORD);

  auto drawHitRow = [&](uint8_t i, int16_t x0, int16_t colW) {
    unsigned long elapsed = rec.hits[i].hit_time_ms;
    unsigned long split   = (i == 0) ? elapsed
                           : elapsed - rec.hits[i-1].hit_time_ms;

    // Pass/Fail 判斷（第一發永遠 pass，無 drill 時永遠 pass）
    bool pass = true;
    if (passThresh > 0 && i > 0)
      pass = (split <= passThresh);

    char elBuf[8]; fmtTime(elapsed, elBuf, sizeof(elBuf));
    char spBuf[8]; fmtTime(split,   spBuf, sizeof(spBuf));

    uint8_t localRow = twoCol ? (i < 5 ? i : i - 5) : i;
    int16_t iy  = LIST_Y + localRow * ROW_H;
    int16_t icy = iy + ROW_H / 2;

    // 背景 highlight（FAIL = 暗紅）
    uint16_t rowBg = (!pass) ? 0x3000 : TFT_BLACK;
    M5.Lcd.fillRect(x0, iy, colW, ROW_H - 1, rowBg);

    // 序號（左對齊）
    M5.Lcd.setTextDatum(ML_DATUM);
    M5.Lcd.setTextColor(pass ? UIColor::TEXT_DIM : 0xF800, rowBg);
    char idxBuf[4]; snprintf(idxBuf, sizeof(idxBuf), "#%u", i + 1);
    M5.Lcd.drawString(idxBuf, x0 + 2, icy, GFXFF);

    // elapsed（右對齊到欄位 60% 處）
    M5.Lcd.setTextDatum(MR_DATUM);
    M5.Lcd.setTextColor(TFT_WHITE, rowBg);
    char elStr[12]; snprintf(elStr, sizeof(elStr), "%ss", elBuf);
    M5.Lcd.drawString(elStr, x0 + colW * 6 / 10, icy, GFXFF);

    // split（右對齊到欄尾）
    M5.Lcd.setTextDatum(MR_DATUM);
    M5.Lcd.setTextColor(pass ? TFT_CYAN : 0xF800, rowBg);
    char spStr[12]; snprintf(spStr, sizeof(spStr), "+%ss", spBuf);
    M5.Lcd.drawString(spStr, x0 + colW - 2, icy, GFXFF);
  };

  for (uint8_t i = 0; i < col1; i++)
    drawHitRow(i, 0, twoCol ? CW : HW::LCD_W);

  for (uint8_t i = 0; i < col2; i++)
    drawHitRow(5 + i, RX, CW);
}
