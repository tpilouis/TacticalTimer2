...
/* Shot log grid */
.sgrid{background:var(--bord);border-radius:4px;overflow:hidden;margin:0 10px 10px}
.scol{background:var(--bg);display:flex;flex-direction:column}
.shdr{display:grid;grid-template-columns:40px 64px 1fr 88px;column-gap:20px;padding:6px 14px;background:var(--bg3);overflow:hidden;scrollbar-gutter:stable}
.shc{font-size:11px;letter-spacing:2px;color:var(--cyan3);text-transform:uppercase;font-weight:600}
.srows{overflow-y:auto;max-height:320px;scrollbar-gutter:stable}
.srow{display:grid;grid-template-columns:40px 64px 1fr 88px;column-gap:20px;align-items:center;padding:9px 14px;border-bottom:1px solid var(--bg3)}
/* 手機隱藏 SRC 欄 */
@media(max-width:480px){
  .shdr{grid-template-columns:40px 64px 1fr 88px;column-gap:20px}
  .srow{grid-template-columns:40px 64px 1fr 88px;column-gap:20px}
}
...
.rn{font-size:11px;color:var(--cyan3);text-align:center;display:flex;align-items:center;justify-content:center}
.srow-src{display:flex;align-items:center;justify-content:center}
.bdg{font-size:8px;padding:2px 5px;border-radius:2px;white-space:nowrap}
...
.tv{font-size:20px;color:#fff;letter-spacing:2px;line-height:1;transition:color .3s}
.tv small{font-size:9px;color:var(--cyan2);margin-left:1px}
.sp{font-size:14px;text-align:right;letter-spacing:1px;transition:color .3s}
...
.set-row{display:flex;align-items:center;justify-content:space-between;padding:12px 0;border-bottom:1px solid var(--bord2);gap:10px}
...
.hitem-wrap{display:flex;align-items:stretch;margin:0 10px 8px;border-radius:5px}
.hitem{flex:1;background:var(--bg3);border:1px solid var(--bord);border-radius:0 5px 5px 0;padding:11px 12px;cursor:pointer;display:flex;align-items:center;justify-content:space-between;transition:background .15s}
...
.hbar{display:flex;align-items:center;justify-content:space-between;padding:0 10px 10px;gap:8px}
.hsel-all{display:flex;align-items:center;gap:6px;cursor:pointer;font-size:9px;letter-spacing:2px;color:var(--cyan2);padding:6px 0;user-select:none}
.hsel-all input{accent-color:var(--cyan);width:14px;height:14px;cursor:pointer}
.hbdel{padding:6px 14px;background:transparent;border:1px solid #2a0808;border-radius:3px;color:#703030;font-size:9px;letter-spacing:2px;cursor:pointer;font-family:'Courier New',monospace;transition:all .15s;white-space:nowrap}
.hbdel.active{border-color:#8a2020;color:#e05050}
.hbdel.active:active{background:#1a0000}
/* checkbox in list items */
.hck{display:flex;align-items:center;padding:0 10px;cursor:pointer}
.hck input{accent-color:var(--cyan);width:16px;height:16px;cursor:pointer;flex-shrink:0}
.hitem-wrap.checked .hitem{background:var(--bg4);border-color:var(--bord2)}
.hitem-wrap.checked{outline:1px solid #1a4a6a;outline-offset:-1px;border-radius:5px}
.hempty{text-align:center;padding:40px 20px;font-size:13px;letter-spacing:3px;color:var(--cyan2)}
...
.ota-btn{display:inline-block;padding:12px 24px;background:#00d4ff;color:#000;border-radius:6px;font-size:13px;letter-spacing:2px;font-weight:bold;cursor:pointer;user-select:none;box-shadow:0 0 12px rgba(0,212,255,.4)}
.ota-btn:active{opacity:.8}
.ota-bar-wrap{height:12px;background:#0a2a38;border:1px solid #00d4ff;border-radius:6px;overflow:hidden;margin:10px 0 4px}
.ota-bar{height:100%;width:0%;background:linear-gradient(90deg,#00d4ff,#00ffcc);transition:width .25s}
...
<div id="app">
  ...
  <div id="panels">
    ...
    <div class="panel" id="panel-live">
      ...
      <!-- Shot log (single column, infinite scroll) -->
      <div class="sgrid">
        <div class="scol">
          <div class="shdr"><div class="shc" style="text-align:center">#</div><div class="shc" style="text-align:center">SRC</div><div class="shc">TIME</div><div class="shc" style="text-align:right">SPLIT</div></div>
    ...
    <div class="panel" id="panel-sett">
      ...
      <div class="card">
        <div class="ctitle">MAX SHOTS (FREE / RO)</div>
        <div class="thr-block">
          <div class="thr-row">
            <input type="range" id="ms-slider" min="0" max="50" step="1" value="0" oninput="updMaxShots(this.value)">
            <div><span class="thr-val" id="ms-val">INF</span></div>
          </div>
          <div class="hint-row"><span>0 = unlimited (ring buffer)</span><span>max 50</span></div>
        </div>
      </div>
      <div class="card">
    ...
    <div class="panel" id="panel-sys">
      ...
      <!-- Power & System Console -->
      <div class="sys-card">
        <div class="ctitle" style="padding:10px 14px 8px;margin:0;border-bottom:1px solid var(--bord2)">
          ⚡ POWER &amp; SYSTEM LOG
          <span style="float:right;cursor:pointer;font-size:10px;color:var(--dim)" onclick="clearPwrLog()">CLEAR</span>
        </div>
        <div id="pwr-status" style="display:flex;gap:16px;padding:8px 14px;flex-wrap:wrap;border-bottom:1px solid var(--bord2)">
          <div><div style="font-size:8px;color:var(--dim)">BATTERY</div><div id="pw-bat" style="font-size:14px;color:var(--cyan2)">--</div></div>
          <div><div style="font-size:8px;color:var(--dim)">CURRENT</div><div id="pw-cur" style="font-size:14px;color:var(--cyan2)">--</div></div>
          <div><div style="font-size:8px;color:var(--dim)">USB</div><div id="pw-usb" style="font-size:14px;color:var(--cyan2)">--</div></div>
          <div><div style="font-size:8px;color:var(--dim)">AXP TEMP</div><div id="pw-tmp" style="font-size:14px;color:var(--cyan2)">--</div></div>
          <div><div style="font-size:8px;color:var(--dim)">UPTIME</div><div id="pw-upt" style="font-size:14px;color:var(--cyan2)">--</div></div>
        </div>
        <div id="pwr-log" style="font-family:monospace;font-size:10px;color:#4ade80;padding:6px 10px;max-height:160px;overflow-y:auto;white-space:pre-wrap;word-break:break-all;background:#0a1a0f;border-radius:0 0 4px 4px"></div>
      </div>
      <!-- Version info -->
      ...
      <!-- ── Firmware OTA Update (方案 A) ── -->
      <div class="sys-card" style="margin-bottom:16px">
        <div class="ctitle" style="padding:10px 14px 8px;margin:0;border-bottom:1px solid var(--bord2)">&#128736; FIRMWARE UPDATE</div>
        <div style="padding:14px 16px">
          <div style="font-size:13px;color:#cce8f0;line-height:1.7;margin-bottom:14px">
            Select compiled <b style="color:#00d4ff;font-size:14px">TacticalTimer2.ino.bin</b><br>
            <span style="font-size:12px;color:#88bbcc">Device reboots automatically after upload.</span>
          </div>
          <div id="ota-idle">
            <label class="ota-btn" for="ota-file">&#128194;&nbsp; SELECT .BIN FILE</label>
            <input type="file" id="ota-file" accept=".bin" style="display:none" onchange="otaStart(this)">
          </div>
          <div id="ota-prog" style="display:none">
            <div style="font-size:13px;letter-spacing:2px;color:#00d4ff;margin-bottom:4px;font-weight:bold" id="ota-status">&#9654; UPLOADING...</div>
            <div class="ota-bar-wrap"><div class="ota-bar" id="ota-bar"></div></div>
            <div style="font-size:12px;color:#00d4ff;text-align:right;margin-top:2px" id="ota-pct">0%</div>
          </div>
          <div id="ota-ok" style="display:none;font-size:14px;color:#00ff88;letter-spacing:1px;font-weight:bold;padding:8px 0">&#10003; UPDATE COMPLETE &mdash; Rebooting...</div>
          <div id="ota-fail" style="display:none">
            <div style="font-size:14px;color:#ff4444;letter-spacing:1px;font-weight:bold" id="ota-msg">&#10007; UPDATE FAILED</div>
            <button onclick="otaReset()" style="margin-top:10px;padding:10px 20px;background:#00d4ff;color:#000;border:none;border-radius:5px;font-size:13px;font-weight:bold;letter-spacing:1px;cursor:pointer">TRY AGAIN</button>
          </div>
        </div>
      </div>
    </div>
...
<div id="hist-overlay" style="position:fixed;inset:0;background:var(--bg);z-index:99;display:none;flex-direction:column">
  ...
  <div style="flex:1;overflow-y:auto">
    <!-- Filter tags + batch delete toolbar -->
    <div class="hfilter">
    ...
    <div class="hbar">
      <label class="hsel-all">
        <input type="checkbox" id="hck-all" onchange="toggleAllCheck(this.checked)">
        SELECT ALL
      </label>
      <button class="hbdel" id="hbdel-btn" onclick="batchDelete()">🗑 DELETE (<span id="hsel-count">0</span>)</button>
    </div>
...
// ── Settings ─────────────────────────────────────────────────
var pSrc=1,pThr=2000,pMaxShots=0;
function applySett(){
  fetch('/setting',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({hitSource:pSrc,micThresh:pThr,maxShots:pMaxShots})})
...
function updMaxShots(v){
  pMaxShots=parseInt(v);
  document.getElementById('ms-val').textContent=(pMaxShots===0)?'INF':pMaxShots;
  applySett();
}
...
function appendPwrLog(msg){
  var el=document.getElementById('pwr-log');if(!el)return;
  var ts=new Date().toLocaleTimeString();
  var color=msg.indexOf('WARNING')>=0||msg.indexOf('LOW')>=0?'#f87171':
            msg.indexOf('Low')>=0||msg.indexOf('NOTICE')>=0?'#fbbf24':'#4ade80';
  el.innerHTML+='<span style="color:#6b7280">['+ts+']</span> <span style="color:'+color+'">'+msg+'</span>\n';
  el.scrollTop=el.scrollHeight;
}
function clearPwrLog(){
  var el=document.getElementById('pwr-log');if(el)el.textContent='';
}
function clearLog(){
  _hitSeen={};
  ['L'].forEach(function(id){document.getElementById(id).innerHTML='<div class="empty">--- WAITING ---</div>'});
  updShotsEl(0);
...
function shotsMax(){return curMode===3?dShots:(pMaxShots||0)}
...
function addShot(d){
  ...
  var col=document.getElementById('L');  // single column, unlimited scroll
...
function onItemCheck(ck){
  ck.closest('.hitem-wrap').classList.toggle('checked',ck.checked);
  updateSelCount();
  // 更新全選狀態
  var all=document.querySelectorAll('.hitem-ck');
  var checked=document.querySelectorAll('.hitem-ck:checked');
  document.getElementById('hck-all').checked=all.length>0&&checked.length===all.length;
}
function toggleAllCheck(checked){
  document.querySelectorAll('.hitem-ck').forEach(function(ck){
    ck.checked=checked;
    ck.closest('.hitem-wrap').classList.toggle('checked',checked);
  });
  updateSelCount();
}
function updateSelCount(){
  var n=document.querySelectorAll('.hitem-ck:checked').length;
  document.getElementById('hsel-count').textContent=n;
  var btn=document.getElementById('hbdel-btn');
...
function batchDelete(){
  ...
  var paths=Array.from(checked).map(function(ck){return ck.dataset.path});
  // 使用現有確認 dialog
  var msg=n===1?'Delete 1 session?':'Delete '+n+' sessions?';
  if(!confirm(msg))return;
  // 逐筆刪除（串行）
  var promise=Promise.resolve();
  paths.forEach(function(path){
    promise=promise.then(function(){
      return fetch('/history/delete',{method:'POST',
  ...
  promise.then(function(){
    // 從本地列表移除已刪除項目
    histAll=histAll.filter(function(s){return paths.indexOf(s.path)<0});
    renderHistList();
    // 重設全選狀態
    document.getElementById('hck-all').checked=false;
    updateSelCount();
  }).catch(function(){alert('Some deletions failed. Please refresh.')});
}
function loadHist(){
  var list=document.getElementById('hlist');
  list.innerHTML='<div class="hempty">Loading...</div>';
  var ca=document.getElementById('hck-all');if(ca)ca.checked=false;
  updateSelCount();
...
function renderHistList(){
  ...
  if(!items.length){list.innerHTML='<div class="hempty">'+(histAll.length?'No '+histFilter+' sessions':'No sessions recorded')+'</div>';updateSelCount();return}
  list.innerHTML='';
  items.forEach(function(s){
    ...
    w.dataset.path=s.path;
    w.dataset.mode=s.mode||'';
    w.innerHTML=
      // checkbox column
      '<label class="hck" onclick="event.stopPropagation()">'+
        '<input type="checkbox" class="hitem-ck" onchange="onItemCheck(this)" data-path="'+safePath+'">'+
      '</label>'+
      // main item (click = open detail)
...
function loadState(){
  fetch('/state').then(function(r){return r.json()}).then(function(d){
    ...
    document.getElementById('par-sub').textContent=curPar?(fx(curPar)+'s limit'):'Disabled';
    // settings
    pSrc=d.hitSource||1;pThr=d.micThresh||2000;pMaxShots=d.maxShots||0;
    document.getElementById('ms-val').textContent=(pMaxShots===0)?'INF':pMaxShots;
    document.getElementById('ms-slider').value=pMaxShots;
...
if(!!window.EventSource){
  ...
  function connect(){
    ...
    s.addEventListener('log',    function(e){appendPwrLog(e.data)});
    s.addEventListener('power',  function(e){
      var d=JSON.parse(e.data);
      var batColor=d.v<3.3?'#f87171':d.v<3.6?'#fbbf24':'var(--cyan2)';
      document.getElementById('pw-bat').innerHTML='<span style="color:'+batColor+'">'+d.v.toFixed(3)+'V ('+d.pct+'%)</span>';
      document.getElementById('pw-cur').textContent=(d.i>=0?'+':'')+d.i.toFixed(0)+'mA '+(d.i<0?'⚡CHG':d.i>10?'▼DSC':'■IDL');
      document.getElementById('pw-usb').textContent=d.vbus>1?d.vbus.toFixed(2)+'V':'--';
      document.getElementById('pw-tmp').textContent=d.t.toFixed(1)+'°C';
      document.getElementById('pw-upt').textContent=fmtUptime(d.up);
    });
...
// ── OTA Firmware Update (方案 A) ─────────────────────────────
function otaReset(){
  document.getElementById('ota-idle').style.display='';
  document.getElementById('ota-prog').style.display='none';
  document.getElementById('ota-ok').style.display='none';
  document.getElementById('ota-fail').style.display='none';
  document.getElementById('ota-file').value='';
}
function otaStart(input){
  var file=input.files[0];
  if(!file) return;
  if(!file.name.endsWith('.bin')){alert('Please select a .bin file');input.value='';return;}
  document.getElementById('ota-idle').style.display='none';
  document.getElementById('ota-prog').style.display='';
  document.getElementById('ota-bar').style.width='0%';
  document.getElementById('ota-pct').textContent='0%';
  document.getElementById('ota-status').textContent='UPLOADING...';
  ...
  xhr.onload=function(){
    document.getElementById('ota-prog').style.display='none';
    if(xhr.status===200) document.getElementById('ota-ok').style.display='';
    else{
      document.getElementById('ota-fail').style.display='';
      document.getElementById('ota-msg').textContent='UPDATE FAILED ('+xhr.status+')';
    }
  };
  xhr.onerror=function(){
    document.getElementById('ota-prog').style.display='none';
    document.getElementById('ota-fail').style.display='';
    document.getElementById('ota-msg').textContent='CONNECTION ERROR';
  };
  var fd=new FormData();
 ...
 */
/**
 * @brief 推播 log 訊息到 Web Dashboard 的 Power Console
 * @param msg  訊息字串
 */
void WebServer::sendLog(const char* msg) {
  if (!msg || msg[0] == '\0') return;
  s_events.send(msg, "log", millis());
}

/**
 * @brief 推播電源狀態到 Web Dashboard
 */
void WebServer::sendPower(float batV, float batI,
                          float vbusV, float axpTemp,
                          uint32_t uptimeSec) {
  int batPct = (int)constrain((batV - 3.0f) / (4.2f - 3.0f) * 100.0f, 0.0f, 100.0f);
  char buf[128];
  snprintf(buf, sizeof(buf),
           "{\"v\":%.3f,\"pct\":%d,\"i\":%.1f,\"vbus\":%.3f,\"t\":%.1f,\"up\":%lu}",
           batV, batPct, batI, vbusV, axpTemp, (unsigned long)uptimeSec);
  s_events.send(buf, "power", millis());
}
 ...
 */
/**
 * @brief 初始化 ArduinoOTA（方案 B：Arduino IDE 同網路無線燒錄）
 * ★ ArduinoOTA.begin() 會強制改成 WIFI_STA，begin() 後立即恢復 WIFI_AP_STA
 */
void WebServer::initOTA() {
  ArduinoOTA.setHostname("TacticalTimer2");
  ArduinoOTA.onStart([]() {
  ...
  ArduinoOTA.begin();
  // ★ 恢復 WIFI_AP_STA（begin() 會強制改成 WIFI_STA）
  WiFi.mode(WIFI_AP_STA);
  if (NetworkMgr::isAPMode()) {
    WiFi.softAP(NetCfg::AP_SSID, NetCfg::AP_PASSWORD);
    Serial.println("[ArduinoOTA] AP mode restored");
  }
...
void WebServer::init() {
  ...
  s_server.on("/state", HTTP_GET, [](AsyncWebServerRequest* req) {
    ...
    doc["maxShots"]    = cfg.maxShots;     // Free/RO 最大發數（0=無限）
  ...
  s_server.on("/setting", HTTP_POST, [](AsyncWebServerRequest* req){},
    ...
    [](AsyncWebServerRequest* req, uint8_t* data, size_t len, size_t, size_t) {
      ...
      if (doc.containsKey("maxShots"))  s.maxShots  = (uint8_t)constrain((int)(doc["maxShots"]|0), 0, (int)Limits::HIT_MAX);
      TimerCore::setMaxShots(s.maxShots);
      HalStorage::saveSettings(s);
      ...
      req->send(200);
      Serial.printf("[WebServer] Setting: src=%d thr=%d maxShots=%d preset=%d\n",
                    (int)s.hitSource, s.micThresh, s.maxShots, idx);
    }
  );
  ...
  s_server.on("/update", HTTP_POST,
    [](AsyncWebServerRequest* req) {
      ...
      req->send(resp);
      if (ok) { Serial.println("[OTA] Complete, rebooting..."); delay(500); ESP.restart(); }
    },
