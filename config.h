...
namespace TT2Version {
  ...
  constexpr char    TT2_APP_VER[]   = "2.0.0";
...
// ============================================================
namespace Limits {
  constexpr uint8_t  HIT_MAX       = 10;   ///< 每局最多擊發數
...
namespace NVS {
  ...
  constexpr uint8_t  FMT_VER_VAL  = 1;
}
...
TT2_STATIC_ASSERT(sizeof(SseMsg)     == 10, "SseMsg size mismatch");       // 1+1+4+4
TT2_STATIC_ASSERT(sizeof(AudioCmd)   <= 28, "AudioCmd too large for queue");
TT2_STATIC_ASSERT(Limits::HIT_MAX   <= 32,  "HIT_MAX exceeds safe range");
