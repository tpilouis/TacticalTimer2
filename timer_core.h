...
namespace TimerCore {
   ...
   */
  void forceStop();
  void abortSilent();   ///< 靜默中止：直接跳 IDLE，不觸發 SHOWCLEAR/音效
   ...
   *                 nullptr = 略過回呼
   */
  void processHits(void (*onHitCb)(uint32_t, uint8_t,
  ...
  void     setMaxShots(uint8_t n);   ///< 0=無限（環形），1-50=到達後自動停
  uint8_t  getMaxShots();
  uint32_t getTotalShots();          ///< 本局真實總命中數（環形覆寫時仍累加）
