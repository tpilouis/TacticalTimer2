...
void ModeSpy::onEnter() {
  ...
  // 啟動 Spy Mode：MicEngine 不再需要 GOING 狀態
...
void ModeSpy::onButton(uint8_t btn) {
  switch (btn) {
    case 0:  // BtnA — 停止 / Home
      if (_listening) { stopListening(); drawScreen(); }
...
void ModeSpy::drawScreen() {
  ...
  unsigned long totalMs = (_firstShotAt > 0 && _shotCount > 0)
                          ? _lastShotAt - _firstShotAt : 0;
  UIScreen::drawSpyScreen(_shotCount, totalMs, _hits, _listening);
  UIScreen::drawBtnBar(_listening ? "STOP" : "HOME",
                       "CLEAR",
                       _listening ? "PAUSE" : "LISTEN");
}
 ...
 * 此函式保留供未來擴充使用。
 */
void ModeSpy::onSpyHit(uint8_t, uint8_t, unsigned long, unsigned long) {
  // Spy 模式的命中在 update() 中直接從 hitQueue 消費
  // 此 callback 預留給未來擴充
}
