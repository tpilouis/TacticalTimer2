...
void ModeFree::onEnter() {
  s_freeInstance = this;
  _resultShown = false;
  // 確保狀態為 IDLE（靜默中止，不觸發 SHOWCLEAR/音效）
...
void ModeFree::onButton(uint8_t btn) {
  ...
  switch (btn) {
    case 0:  // BtnA — STOP / 上一頁（STOP 時） / 返回 Home（IDLE）
      if (gs == GameState::GOING || gs == GameState::AREYOUREADY ||
          gs == GameState::BEEPING || gs == GameState::WAITING) {
        stopAudio();
        TimerCore::forceStop();
        WebServer::sendGameState();
      } else if (gs == GameState::STOP) {
      ...
      } else if (gs == GameState::IDLE) {
        GameFSM::requestSwitch(AppMode::HOME);
      } else {
        Serial.printf("[ModeFree] BtnA ignored, gs=%u\n", (uint8_t)gs);
      }
      break;
    ...
    case 2:  // BtnC — START / RESTART / 下一頁（STOP 時）
      if (gs == GameState::STOP && UIScreen::shotPageHasNext()) {
        ...
        Serial.printf("[ModeFree] Start -> gameIdx=%u maxShots=%u\n",
                      gi, eff.maxShots);
      } else {
        Serial.printf("[ModeFree] BtnC ignored, gs=%u\n", (uint8_t)gs);
      }
      break;
 ...
 */
void ModeFree::onHit(uint32_t hitIdx, uint8_t stationId,
...
/**
 * @brief 翻頁後重繪當前頁的所有成績列（BtnA/BtnC 翻頁時呼叫）
 *
 * 從 TimerCore 取得 GameRecord，重繪目前 s_pageBase 對應的那一頁。
 * 同時更新按鈕列，顯示翻頁提示。
 */
void ModeFree::redrawShotPage() {
  const GameRecord* rec = TimerCore::getRecord(0);
  if (!rec) return;

  // 只繪製目前頁面的 shot rows（不清畫面、不畫 header、不畫 btn bar）
  ...
  for (uint32_t i = pageBase; i < pageEnd && i <= total; i++) {
    uint32_t slot = (i <= Limits::HIT_MAX) ? (i - 1) : ((i - 1) % Limits::HIT_MAX);
...
// 翻頁完整重繪（header + rows + result stats + btn bar）
void ModeFree::redrawFullPage() {
  const GameRecord* rec = TimerCore::getRecord(0);
  ...
  UIScreen::clearGameArea();
  UIScreen::drawModeHeader("FREE SHOOTING", "RESULT", 0x5AB0);
  redrawShotPage();
  ...
  UIScreen::drawBtnBar(lblA, "PAR SET", lblC);
}
...
void ModeFree::onClearDone() {
  ...
  // 跳到最後一頁成績，重繪完整畫面
  if (s_freeInstance) s_freeInstance->redrawFullPage();
  WebServer::sendGameState();
}
