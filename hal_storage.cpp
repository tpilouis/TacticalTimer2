 ...
 *
 * Preset 以 field-by-field（putString/putShort/putUChar/putULong）儲存，
 * 避免 struct alignment 改變導致讀取錯誤。
...
bool HalStorage::loadSettings(AppSettings& out) {
  ...
  out.maxShots     = prefs.getUChar(NVS::MAX_SHOTS,       0);
...
bool HalStorage::saveSettings(const AppSettings& in) {
  ...
  prefs.putUChar(NVS::MAX_SHOTS,      in.maxShots);
...
/**
 * @brief 從 NVS 讀取 Preset（field-by-field，避免 struct alignment 問題）
 *
...
bool HalStorage::loadPreset(uint8_t idx, Preset& out) {
  ...
  // 各欄位的 NVS key（加上 idx 後綴）
  char kName[24], kSrc[24], kThr[24], kCool[24];
  snprintf(kName, sizeof(kName), "pr%u_name",  (unsigned)idx);
  snprintf(kSrc,  sizeof(kSrc),  "pr%u_src",   (unsigned)idx);
  snprintf(kThr,  sizeof(kThr),  "pr%u_thr",   (unsigned)idx);
  snprintf(kCool, sizeof(kCool), "pr%u_cool",  (unsigned)idx);
  ...
  // 用 name 是否存在判斷此 preset 是否曾被儲存
  bool exists = prefs.isKey(kName);
  if (exists) {
    prefs.getString(kName, out.name, sizeof(out.name));
    out.hitSource  = static_cast<HitSource>(
                       prefs.getUChar(kSrc, static_cast<uint8_t>(HitSource::MIC)));
    out.micThresh  = static_cast<int16_t>(
                       prefs.getShort(kThr, DEFAULT_PRESET_THRESH[idx]));
    out.cooldownMs = prefs.getULong(kCool, MicCfg::COOLDOWN_MS);
  }
  prefs.end();
...
/**
 * @brief 將 Preset 寫入 NVS（field-by-field，避免 struct alignment 問題）
 * @param idx  Preset 索引
...
bool HalStorage::savePreset(uint8_t idx, const Preset& in) {
  ...
  prefs.putString(kName, in.name);
  prefs.putUChar (kSrc,  static_cast<uint8_t>(in.hitSource));
  prefs.putShort (kThr,  in.micThresh);
  prefs.putULong (kCool, in.cooldownMs);
  prefs.end();

  Serial.printf("[HalStorage] Preset %u saved: '%s' thresh=%d src=%d\n",
                idx, in.name, in.micThresh, (int)in.hitSource);
  return true;
}
  ...
  if (storedVer != NVS::FMT_VER_VAL) {
    ...
    if (prefs.begin(NVS::NVS_NS, false)) {
      ...
      for (uint8_t i = 0; i < Limits::PRESET_MAX; i++) {
        // 清除舊格式 key（putBytes 版本）
        char oldKey[24];
        snprintf(oldKey, sizeof(oldKey), "%s%u", PRESET_KEY_PREFIX, (unsigned)i);
        prefs.remove(oldKey);
        // 清除新格式 key（field-by-field 版本）
        char kName[24], kSrc[24], kThr[24], kCool[24];
        snprintf(kName, sizeof(kName), "pr%u_name", (unsigned)i);
        snprintf(kSrc,  sizeof(kSrc),  "pr%u_src",  (unsigned)i);
        snprintf(kThr,  sizeof(kThr),  "pr%u_thr",  (unsigned)i);
        snprintf(kCool, sizeof(kCool), "pr%u_cool", (unsigned)i);
        prefs.remove(kName);
        prefs.remove(kSrc);
        prefs.remove(kThr);
        prefs.remove(kCool);
