...
namespace WebServer {
   ...
   */
  void init();
  void initOTA();  ///< ArduinoOTA 初始化（方案 B：同網路無線燒錄）

  /** @brief 推播 GameState 整數（SSE event: "gs"） */
  void sendGameState();
  void sendLog(const char* msg);              ///< 推播 log 訊息到 Web Console
  void sendPower(float batV, float batI,
                 float vbusV, float axpTemp,
                 uint32_t uptimeSec);         ///< 推播電源狀態到 Web Dashboard
