...
void ModeHistory::loadDetail(uint8_t localIdx) {
  ...
  UIScreen::resetShotPage();  // 新 session 重設分頁
  _phase = HistPhase::DETAIL;
...
void ModeHistory::drawDetailScreen() {
  ...
  const char* lblA = UIScreen::shotPageHasPrev() ? "< PREV" : "BACK";
  const char* lblC = UIScreen::shotPageHasNext() ? "NEXT >" : "DELETE";
  UIScreen::drawBtnBar(lblA, nullptr, lblC);
}
