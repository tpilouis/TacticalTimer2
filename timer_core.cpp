...
namespace {
  ...
  uint8_t       s_hitIdx     = 0;   ///< 目前局命中計數（0-based 陣列索引上限）
 ...
 */
void TimerCore::processHits(void (*onHitCb)(uint8_t, uint8_t,
  ...
  while (IPC::receiveHit(hm)) {
    ...
    unsigned long elapsed, split;
    {
      ...
      if (s_hitIdx  >= Limits::HIT_MAX)  continue;

      auto& h           = s_records[s_gameIdx - 1].hits[s_hitIdx];
      h.station_id      = hm.station_id;
      h.hit_time_ms     = hm.hit_time_ms;
      s_hitIdx++;
      s_records[s_gameIdx - 1].hit_count = s_hitIdx;
      newHi             = s_hitIdx;

      const GameRecord& rec = s_records[s_gameIdx - 1];
      elapsed = calcElapsed(rec, newHi);
      split   = calcSplit(rec, newHi);

      // 達到 HIT_MAX → 自動切到 SHOWCLEAR
      if (s_hitIdx >= Limits::HIT_MAX) {
        s_state        = GameState::SHOWCLEAR;
        s_pendingState = GameState::SHOWCLEAR;
        s_parActive    = false;
      }
